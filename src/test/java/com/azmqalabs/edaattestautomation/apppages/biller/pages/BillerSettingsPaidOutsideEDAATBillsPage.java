...
public class BillerSettingsPaidOutsideEDAATBillsPage extends BasePage{
	...
	//Function Summary : Navigate to Paid OutsideBillsEdaatBills page 
		...
		try {
			...
			test.log(Status.PASS,"Verify Navigate to Paid OutsideBillsEdaatBills page " + driver.getTitle() +" * Navigate to Paid OutsideBillsEdaatBills page is Pass * " );
			Log.ReportEvent("PASS", " Verify Navigate PaidOutsideEDAATBills Page is dispayed Suceessfull");
			this.takeScreenShot();
		}
		catch (Exception e) {
			e.printStackTrace();
			test.log(Status.FAIL,"View Navigate to Paid OutsideBillsEdaatBills page " + driver.getTitle() +" * Navigate to Paid OutsideBillsEdaatBills page is Fail * " );
			this.takeScreenShot();
		}
		...
		try {
			...
			test.log(Status.PASS,"Verify upload Bulk data " + driver.getTitle() +" * upload Bulk data is Pass * " );
			...
			test.log(Status.FAIL,"Verify upload Bulk data " + driver.getTitle() +" * Verify is Fail * " );
			this.takeScreenShot();
		}
		...
		public void PaidOutsideEdaatErrorMessageValidation(Map<Object, Object> testdatamap) throws InterruptedException, IOException
		{
			...
			try
			{
				if(ExistsCheck(EdaatOR.Biller_Settings_PaidOutsideEdaatSelectFileError)) {
					...
					test.log(Status.PASS, "Paid Outside Edaat Upload BulkData Error Message Exists" + driver.getTitle() + " * Paid Outside Edaat Upload BulkData Error Message Exists * ");	    	
			     }
				...
				test.log(Status.FAIL,"Paid Outside Edaat Verify to upload Bulk data " + driver.getTitle() +" * Paid Outside Edaat Verify to upload Bulk data FAIL * " );
				this.takeScreenShot();
			}
