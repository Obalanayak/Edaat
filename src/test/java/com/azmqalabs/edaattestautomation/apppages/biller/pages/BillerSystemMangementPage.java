...
public void UpdateData(Map<Object,Object> testdatamap,Log Log) {
	try {
		...
		if (Ele == true) {
			test.log(Status.PASS,"Valid Activation info for deactivated biller by maker" + driver.getTitle() + " * Activation info for deactivated biller by maker PASS * ");
        	Log.ReportEvent("PASS", " Verify Data has Been updated by the Maker Suceessfully");
        	this.takeScreenShot();

		}else
			test.log(Status.FAIL,"Valid Activation info for deactivated biller by maker" + driver.getTitle() + " * Activation info for deactivated biller by maker FAIL * ");
	} catch (Exception e) {
		test.log(Status.FAIL, "Valid Activation info for deactivated biller by maker" + driver.getTitle() + " * Activation info for deactivated biller by maker FAIL * ");
		this.takeScreenShot();
...
public void DeactoActInvalid(Map<Object,Object> testdatamap,Log Log) {
	try {
		...
		if (Ele == true) {
			test.log(Status.PASS,"Invalid Activation info for deactivated biller by maker" + driver.getTitle() + " * Invalid Activation info for deactivated biller by maker PASS * ");
        	Log.ReportEvent("PASS", " Verify Not Approved Grid view is Loaded Suceessfully");
        	this.takeScreenShot();

		}else
			test.log(Status.FAIL,"Invalid Activation info for deactivated biller by maker" + driver.getTitle() + " * Invalid Activation info for deactivated biller by maker FAIL * ");
	} catch (Exception e) {
		test.log(Status.FAIL, "Invalid Activation info for deactivated biller by maker" + driver.getTitle() + " * Invalid Activation info for deactivated biller by maker FAIL * ");
		this.takeScreenShot();
	}
...
public void ActivatedbyChecker(Map<Object,Object> testdatamap,Log Log) {
	try {
		...
		if (Ele == true) {
			test.log(Status.PASS," Checker approvroval for account activation" + driver.getTitle() + " * Checker approvroval for account activation PASS * ");
        	Log.ReportEvent("PASS", " Account has been approved by the Checker Suceessfully");
        	this.takeScreenShot();
		}else
			test.log(Status.FAIL,"Checker approvroval for account activation" + driver.getTitle() + " * Checker approvroval for account activation FAIL * ");
	} catch (Exception e) {
		test.log(Status.FAIL, "Checker approvroval for account activation" + driver.getTitle() + " * Checker approvroval for account activation FAIL * ");
		this.takeScreenShot();
	}
...
public void ClickonCloseButton(Map<Object,Object>testdatamap,Log Log) throws InterruptedException {
	...
	test.log(Status.PASS," Checker approvroval for account activation" + driver.getTitle() + " * Click on Close Button Without Doing any Actions By Maker PASS * ");
	Log.ReportEvent("PASS", " Verify Account has Been Deactivated by the Checker Suceessfully");
	this.takeScreenShot();
     ...
     //Function Summary   : Method to ApproveBillerErrorMsg
		...
		public void UpdateBillerdetailsErrorMSGvalidation(Map<Object, Object> testdatamap) throws Exception {
			...
			&&ExistsCheck(EdaatOR.Biller_SystemMgm_Biller_Durationerror))
			{
				...
				Thread.sleep(1000);
				test.log(Status.PASS, "BeneficiaryName,Billerid and Transfer duration ErrorMessage Exists" + driver.getTitle() + " * BeneficiaryName,Billerid and Transfer duration ErrorMessage Exists * ");				
			}
				...
				Thread.sleep(1000);
				test.log(Status.PASS, "CorpNameAr,CorpNameEng and BillCat ErrorMessage Exists" + driver.getTitle() + " * CorpNameAr,CorpNameEng and BillCat ErrorMessage Exists * ");				
			}	
			...
			test.log(Status.FAIL,"Not Approved Biller Management" + driver.getTitle()+ " * Not Approved Biller Management is UnSuccessfull FAIL * " );
			...
			try {
				...
				WebClickUsingJS(EdaatOR.Biller_CorpMGMSuspendYesBtn);
			      if(ExistsCheck(EdaatOR.BillerSystem_Corporate_Accountstatuserrormsg))
			      {
			    	  ...
			    	  test.log(Status.PASS, "Corporate clients Account status ErrorMessage Exists" + driver.getTitle() + " * Account Status ErrorMessage Exists * ");
			      }
			    	  ...
			    	  test.log(Status.PASS, "Corporate clients Account Suspended Reason ErrorMessage Exists" + driver.getTitle() + " * Account Suspended Reason ErrorMessage Exists * ");				
			      }
			      else
			      {
			          test.log(Status.FAIL, "Element Not Exists" + driver.getTitle() + " * Element Not Exists * ");		      
				...
				test.log(Status.FAIL,"Verify Corporate settings button" + driver.getTitle() +" * Verify Corporate settings button is unsuccessfull FAIL * " );
			}
		}
			...
			public void VerifyDailyTransferErrorMsg(Map<Object, Object> testdatamap) {
				try {
					...
					if(ExistsCheck(EdaatOR.Admin_DistReason)&&ExistsCheck(EdaatOR.Biller_DistAttachment)&&ExistsCheck(EdaatOR.Biller_DistDate))
					{
						...
						Thread.sleep(1000);
						test.log(Status.PASS, "Transfer Date,Type/Reason and Attachment ErrorMessage Exists" + driver.getTitle() + " * Transfer Date,Type/Reason and Attachment ErrorMessage Exists * ");
					}
					else
					{
						test.log(Status.FAIL, "Element Not Exists" + driver.getTitle() + " * Element Not Exists * ");
					...
					test.log(Status.FAIL,"Verify Distribution Daily transfer" + driver.getTitle() +" * Verify Distribution Daily transfer View is unsuccessfull FAIL * " );
				}
	}
